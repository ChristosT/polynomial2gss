...
def ALGO11(As,Bs,Cs,Ds,do_test):
    ...
    PB=Matrix(np.mat(PiB)[np.ix_(I,range(k))])
    PC=Matrix(np.mat(PiC)[np.ix_(range(k),J)])
    #===================END=STEP 3======================================
    #=======================STEP 4======================================
    Lambda=r+rE +p +m
    
    E=SparseMatrix(BlockMatrix([[List_P[1],PC,zeros(PC.rows,Lambda-k-PC.cols)],
    [PB,PA,zeros(PA.rows,Lambda-PB.cols -PA.cols)],
    [zeros(Lambda-k-PB.rows,List_P[1].cols),zeros(Lambda-k-PB.rows,PC.cols),zeros(Lambda-k-PB.rows,Lambda-k-len(J))]]))
    
    A=SparseMatrix(BlockMatrix([[-List_P[0],zeros(k,PE.cols),zeros(k-p,Lambda-k-PE.cols).col_join(-eye(p))],
    [zeros(PE.rows,k),PE,zeros(PE.rows,p)],
    [zeros(m,k-m).row_join(eye(m)),zeros(m,PE.cols),zeros(m,p)]]))
    
    
    B=SparseMatrix(BlockMatrix([[zeros(r+p+rE,m)],[eye(m)]]))
    C=SparseMatrix(BlockMatrix([[zeros(p,r+m+rE),eye(p)]]))
    D=zeros(p,m)
    #===============END==STEP 4=========================================
